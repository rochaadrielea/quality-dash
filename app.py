...
The whole dashboard you already had is UNCHANGED — it now simply lives inside
the first of two top tabs, like the personal dashboard:

    [ 📊 Quality Dashboard ]  [ 🎯 CAPA ]

  * Quality Dashboard — every section you had before (Burndown, Monthly, Quality
    Dashboard, Trends & Breakdown, CAPA Coverage, Root Cause, Per-owner report,
    Data Quality, Validation). Same code, just indented under `with tab_dash:`.
  * CAPA — the new tab: CAPA burnout by launcher class + the zoomable RCA pizza,
    rendered by capa_view.render().

set_page_config is the first Streamlit call (once). The password gate runs once,
so both tabs sit behind the same single login. `run.bat` still runs this file.
...
# Two top tabs. The entire existing dashboard renders inside the first tab; the
# new CAPA view renders inside the second. `with` is not a Python scope, so the
# helper functions the body defines (`_q`, `build_where`, `_qf`, ...) stay
# module-global and keep working exactly as before.
...
with tab_dash:
    ...
    with st.sidebar:
        ...
        st.markdown("""
            ...
            <a href="#my-ncs" class="nav-link">Per-owner report</a>
            <a href="#data-quality" class="nav-link">Data Quality</a>
            <a href="#validation" class="nav-link">Validation Warnings</a>
        """, unsafe_allow_html=True)
        st.markdown("---")
    ...
    # 'Monthly opens vs closes' (grouped bars) removed — it drew exactly the same
    # numbers as 'Opened vs Closed — monthly trend' below, which shows them as lines.
    # The filter it defined is still needed by that chart, so it stays.
    ...
    if not df_trend.empty:
        ...
        fig_tr.update_layout(height=280, margin=dict(l=0, r=0, t=MODEBAR_T, b=0),
                             legend=dict(orientation="h", y=-0.15), xaxis_title="", yaxis_title="NCs")
        st.plotly_chart(fig_tr, width='stretch', config=CHART_CONFIG)
                ...
                if picked and picked != "— select —":
                    ...
                    if not df_fu.empty:
                        ...
                        if not recorded.empty:
                            ...
                            st.plotly_chart(figd, width='stretch', config=CHART_CONFIG,
                                            key="fu_drill_chart")
    ...
    with col_d:
        st.markdown("**NCs by Owner** (Top 10) · :grey[open + no-status]")
        st.caption(f"📅 {date_from} → {date_to}")
        _own_extra = (""", SUM(CASE WHEN is_open IS NULL THEN 1 ELSE 0 END) AS no_status"""
                      if HAS_STATUS_STATE else "")
        df_own = _qf(f"""SELECT COALESCE(owner,'(no owner)') AS owner,
                    COUNT(*) AS total_ncs,
                    SUM(CASE WHEN is_open=1 THEN 1 ELSE 0 END) AS open_ncs{_own_extra},
                    MAX(days_open) AS oldest_days
                    FROM nc {{WHERE}} GROUP BY owner HAVING total_ncs>0
                    ORDER BY total_ncs DESC LIMIT 10""")
        if not df_own.empty:
            _cols = {"owner": "Owner",
                     "total_ncs": st.column_config.NumberColumn("Total"),
                     "open_ncs": st.column_config.NumberColumn("Open"),
                     "oldest_days": st.column_config.NumberColumn("Oldest (days)")}
            if HAS_STATUS_STATE:
                _cols["no_status"] = st.column_config.NumberColumn("No status")
            st.dataframe(df_own, width='stretch', hide_index=True, height=280,
                         column_config=_cols)
            if HAS_STATUS_STATE:
                _ns = int(df_own["no_status"].sum())
                st.caption(
                    f"**Total** = every NC for that owner in range. **Open** = Status says Open. "
                    f"**No status** = the Status cell is blank in the source — neither open nor "
                    f"closed ({_ns} here). Those NCs used to be counted as closed and vanish from "
                    f"this table, which is why owner counts read short. High 'Oldest' may need escalation.")
            else:
        ...
        if _n_total == 0:
            ...
            with st.container(border=True):
                ...
                if view == "Side by side":
                    ...
                    _sel_s = st.plotly_chart(figc, width='stretch', config=CHART_CONFIG,
                    ...
                    if _d == 0:
                        ...
                        _sel = st.plotly_chart(figc, width='stretch', config=CHART_CONFIG,
            ...
            # ---- What the CAPA file itself holds, regardless of matching ----
            # The percentages above are join-based: a CAPA row whose NC number is not
            # in `nc` contributes to nothing. That is correct for coverage, but it
            # hides how much is in the file. These are the raw counts, stated plainly.
            _capa_raw = _q("""
                SELECT capa_type, COUNT(*) AS rows, COUNT(DISTINCT nc_id) AS ncs
                FROM capa GROUP BY capa_type ORDER BY rows DESC""")
            _orphans = _q("""
                SELECT COUNT(DISTINCT c.nc_id) AS n FROM capa c
                LEFT JOIN nc n ON n.nc_id = c.nc_id WHERE n.nc_id IS NULL""").iloc[0]["n"]
            if not _capa_raw.empty:
                with st.expander("What the CAPA tracker file actually contains"):
                    _tot_rows = int(_capa_raw["rows"].sum())
                    _tot_ncs = _q("SELECT COUNT(DISTINCT nc_id) n FROM capa").iloc[0]["n"]
                    st.dataframe(_capa_raw.rename(columns={
                        "capa_type": "Type", "rows": "Rows in file", "ncs": "NCs"}),
                        width='stretch', hide_index=True)
                    st.caption(
                        f"**{_tot_rows:,} rows across {int(_tot_ncs):,} NCs** in the CAPA tracker. "
                        f"The percentages above count only NCs that exist in the NC data — "
                        + (f"**{int(_orphans)} NC number(s) in the CAPA file match nothing** and are "
                           f"excluded from them. They are neither lost nor counted: they are listed "
                           f"in the validation warnings."
                           if _orphans else "every CAPA NC number matches an NC.")
                    )
            ...
            if df_ncdetail.empty:
                ...
                st.dataframe(df_ncdetail, width='stretch', hide_index=True, height=420)
            ...
            if not df_combo.empty:
                ...
                if _cmb.empty:
                    ...
                    st.plotly_chart(figk, width='stretch', config=CHART_CONFIG, key="cov_combo")
                    st.caption("Every NC in range falls into exactly one bar. Red = nothing recorded at all. "
                               "This is the same population as the cards at the top of this section, "
                               "cut a different way.")
            ...
            if not df_o1.empty:
                ...
                if _o_pick != "— select —":
                    ...
                    if not df_o2.empty:
                        ...
                        figd.update_traces(textposition="outside",
                                           hovertemplate="<b>%{y}</b><br>NCs: %{x}<extra></extra>")
                        st.plotly_chart(figd, width='stretch', config=CHART_CONFIG, key="o2_chart")
            ...
            if not df_r1.empty:
                ...
                if _r_pick != "— select —":
                    ...
                    if not df_r2.empty:
                        ...
                        figd.update_traces(textposition="outside",
                                           hovertemplate="<b>%{y}</b><br>NCs: %{x}<extra></extra>")
                        st.plotly_chart(figd, width='stretch', config=CHART_CONFIG, key="r2_chart")
    ...
    # ══════════════════════════════════════════════════════════════════════════════
    # MY NCs — one owner's whole picture, in a form you can send them
    # ══════════════════════════════════════════════════════════════════════════════
    df_mine = None
    df_mywarn = None
    st.markdown('<div id="my-ncs"></div>', unsafe_allow_html=True)
    st.subheader("Per-owner report")
    st.caption("Everything for one person: their NCs, what is missing on each, and the "
               "warnings raised against their rows. Nobody else's data appears here — "
               "this is the sheet you can send them as-is.")

    _own_list = _q("SELECT DISTINCT owner FROM nc WHERE owner IS NOT NULL ORDER BY owner")["owner"].tolist()
    if not _own_list:
        st.info("No owners in the data. Owner comes from the tracker only — the SAP "
                "export has no owner field.")
    else:
        _pick_owner = st.selectbox("Owner", ["— select —"] + _own_list, key="my_owner",
                                   help="Owner is recorded in the NC tracker. SAP-only NCs "
                                        "have no owner and will not appear for anyone.")
        if _pick_owner != "— select —":
            # Deliberately NOT date-filtered: someone asking 'what do I have' wants
            # all of it, not a window. The sidebar From/To would silently hide rows
            # and restart the very argument this page exists to settle.
            _mine_sql = """
                SELECT n.nc_id AS "NC number", n.tc_id AS "TC ID",
                       COALESCE(n.project,'—') AS "Project",
                       COALESCE(n.flight_unit,'—') AS "Flight Unit",
                       ...
                       n.source AS "Source"
                FROM nc n WHERE n.owner = ? ORDER BY n.created_on DESC
            """ if HAS_STATUS_STATE else """
                SELECT n.nc_id AS "NC number", n.tc_id AS "TC ID",
                       COALESCE(n.project,'—') AS "Project",
                       COALESCE(n.flight_unit,'—') AS "Flight Unit",
                ...
                FROM nc n WHERE n.owner = ? ORDER BY n.created_on DESC
            """
            df_mine = _q(_mine_sql, [_pick_owner])

            _n_all = len(df_mine)
            _n_open = int((df_mine["Status"] == "Open").sum())
            _n_closed = int((df_mine["Status"] == "Closed").sum())
            _n_nostat = int((df_mine["Status"] == "(no status)").sum())

            k1, k2, k3, k4 = st.columns(4)
            k1.markdown(_kpi_card("Total NCs", _n_all,
                                  f"Every NC assigned to {_pick_owner} — all dates, no filter."),
                        unsafe_allow_html=True)
            k2.markdown(_kpi_card("Open", _n_open, "Status says Open."), unsafe_allow_html=True)
            k3.markdown(_kpi_card("Closed", _n_closed, "Status says Closed."), unsafe_allow_html=True)
            k4.markdown(_kpi_card("No status", _n_nostat,
                                  "Status cell is blank — neither open nor closed. "
                                  "These used to be counted as closed and disappeared."),
                        unsafe_allow_html=True)

            st.caption(f"**{_n_all}** NCs for **{_pick_owner}**. This section ignores the "
                       f"sidebar date filter on purpose — it answers 'what do I have', "
                       f"which should not depend on a date window.")
            st.dataframe(df_mine, width='stretch', hide_index=True, height=340)

            # ---- What is missing on their rows ----
            ...
            if not _gaps.empty:
                ...
                st.dataframe(_gaps, width='stretch', hide_index=True)

            # ---- Their warnings, and only theirs ----
            if _has_table("warnings"):
                _wcols = _q("PRAGMA table_info(warnings)")["name"].tolist()
                if "owner" in _wcols:
                    df_mywarn = _q(
                        "SELECT check_name AS 'Check', subject AS 'NC', severity AS 'Severity', "
                        "message AS 'What it means', first_seen AS 'First seen' "
                        "FROM warnings WHERE owner = ? ORDER BY severity, first_seen",
                        [_pick_owner])
                    st.markdown(f"**Warnings raised against {_pick_owner}'s rows**")
                    if df_mywarn.empty:
                        st.success("None — nothing flagged on their NCs.")
                    else:
                        st.dataframe(df_mywarn, width='stretch', hide_index=True)
                        st.caption("Only warnings tied to this person's NCs. Pipeline-level "
                                   "problems (a renamed column, an unreadable file) are not "
                                   "shown — they are not theirs to fix.")

            # ---- One workbook for this person ----
            _pkg = {"My_NCs": df_mine}
            if not _gaps.empty:
                _pkg["Missing_Fields"] = _gaps
            if df_mywarn is not None and not df_mywarn.empty:
                _pkg["My_Warnings"] = df_mywarn
            st.download_button(
            ...
            st.caption("One workbook, one person: their NCs, their gaps, their warnings. "
                       "Safe to send — it contains nobody else's rows.")
    ...
    st.dataframe(incomplete, width='stretch', hide_index=True, height=300)
    ...
    if HAS_CAPA_TYPE:
        ...
        st.dataframe(rca_blank, width='stretch', hide_index=True, height=240)
        st.download_button("📥 Excel", to_excel_bytes(rca_blank, "RCA_Missing_Cause"),
                           "rca_missing_cause.xlsx",
                           "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                           key="dl_rca_blank")

    # ---- Validation warnings: what the pipeline noticed about the source files ----
    df_warn = None
    if _has_table("warnings"):
        st.markdown("---")
        st.markdown('<div id="validation"></div>', unsafe_allow_html=True)
        st.markdown("#### 🔎 Validation warnings")
        df_warn = _q("SELECT * FROM warnings ORDER BY severity, first_seen DESC")
        if df_warn.empty:
            st.success("No warnings — the source files passed every check.")
        else:
            _n_err = int((df_warn["severity"] == "error").sum())
            _n_warn = int((df_warn["severity"] == "warn").sum())
            _n_info = int((df_warn["severity"] == "info").sum())

            wc1, wc2, wc3 = st.columns(3)
            wc1.metric("Errors", _n_err, help="Something the pipeline depends on is broken or missing.")
            wc2.metric("Warnings", _n_warn, help="Data problems in the source files — blank status, missing IDs, owner spellings.")
            wc3.metric("Info", _n_info, help="Worth knowing, not a problem. New columns, for example.")

            _sev_pick = st.multiselect(
                "Show", ["error", "warn", "info"],
                default=[s for s in ["error", "warn"] if s in df_warn["severity"].values],
                key="warn_sev")
            _shown = df_warn[df_warn["severity"].isin(_sev_pick)] if _sev_pick else df_warn

            _disp = _shown.rename(columns={
                "check_name": "Check", "subject": "Subject", "severity": "Severity",
                "message": "What it means", "first_seen": "First seen",
                "last_seen": "Last seen", "times_seen": "Seen"})
            st.dataframe(_disp, width='stretch', hide_index=True, height=320)
            st.caption(
                "Found by `validate.py` when reading the source Excel files, before anything "
                "was written. **First seen** is when the problem first appeared and never "
                "changes — a repeat only bumps **Seen**. Nothing is deleted, so a warning that "
                "stops occurring stays here with its last date frozen. "
                "These warnings do not block the ingest.")
            st.download_button("📥 Excel", to_excel_bytes(_disp, "Validation_Warnings"),
                               "validation_warnings.xlsx",
                               "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                               key="dl_warn")
